...
const usernames = ($.isNode() ? process.env.XIAOMI_STEP_USERNAME : $.getdata('xiaomi_step_username')) || '这里填写你的小米帐号，多用户使用&&分割' // 使用&&分割多账号
const passwords = ($.isNode() ? process.env.XIAOMI_STEP_PASSWORD : $.getdata('xiaomi_step_password')) || '这里填写密码，多用户使用&&分割' // 同上
const space = ($.isNode() ? process.env.XIAOMI_STEP_SPACE : $.getdata('xiaomi_step_space')) || '10000-49999' // 区间: 使用-分隔使用&进行分割，如果存在&则匹配每个账号
